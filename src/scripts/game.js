...
import levelEncode from "./levels/encodelevel";
...
function Game(opts) {
    ...
    function resize() {
        ...
        tileSize =
            ((canvas.width + canvas.height) / 2 - 30) / (opts.wMod || opts.w);
        tileHalf = tileSize / 2;
    ...
    var viewport = [
        canvas.width / 2,
        canvas.height / 2 - mapHeight + tileHalf / 4,
    ];
    var renderChrome = 1;
    ...
    tileQueueBounds = getTileQueuePos(0);

    // Crete map
    map = crawlMap([], opts.w, opts.h, function () {
        return opts.base;
    });
    ...
    // Create a graph of overrides so we can mess with tiles.
    points = crawlMap([], opts.w, opts.h, function () {
        return [0, 0];
    });
    ...
    function touchstart(e) {
        ...
        lastHoveredTilePos = getIsometricPos(
            lastHoveredTileCoords[0],
            lastHoveredTileCoords[1],
            tileSize,
        );
        ...
        if (!selectedTile) {
            longPress = setTimeout(function () {
                ...
                if (
                    moves < 10 &&
                    touchCurrentSpot - 15 < touchStartSpot &&
                    touchCurrentSpot + 15 > touchStartSpot
                ) {
                    displayOffset = 0;
    ...
    function touchend(e) {
        ...
        if (selectedTile) {
            if (
                ...
                gameIsFree ||
                canPlaceTileHere(selectedTile, lastHoveredTileCoords)
            ) {
    ...
    function canPlaceTileHere(placingThis, coords) {
        ...
        for (var i = 0; i < 2; i++) {
            if (
                jsonStringify(coords) ===
                jsonStringify(opts.predef[i].slice(0, 2))
            ) {
                return false;
            }
        }
    ...
    function getPixelPosFromTouch(touch, includeOffset) {
        var pp = getPixelPos(
            touch.clientX - viewport[0],
            touch.clientY -
                viewport[1] -
                (includeOffset !== 0 ? displayOffset : 0),
            tileSize,
        );
    ...
    function fallAway(currentDrawPos) {
        var offset =
            (Math.pow(1.02, Math.max(0, (now - this.start) / 3)) / 200) *
            canvas.height;
    ...
    function calculatePoints(here) {
        ...
        trafficDirections.forEach(function (dir) {
            var pos = [here[0] + dir[1][0], here[1] + dir[1][1]];
            try {
    ...
    function calculateTrafficPath(here, there, lastMove, path) {
        ...
        var oppositeThisMovement =
            lastMove[0] > 1 ? 0 - 2 + lastMove[0] : lastMove[0] + 2;
        var nextMovement;
        ...
        if (thisTileSpec.c[lastMove[0]]) {
            ...
            var possibleMovements = trafficDirections.filter(function (option) {
                if (
                    thisTileSpec.c[option[0]] &&
                    option[0] !== oppositeThisMovement
                ) {
                    return true;
                }
            });
        ...
        var oppositeNextMovement =
            nextMovement[0] > 1 ? 0 - 2 + nextMovement[0] : nextMovement[0] + 2;
        if (
            nextTileSpec &&
            nextTileSpec.c &&
            nextTileSpec.c[oppositeNextMovement]
        ) {
    ...
    function cacheSprite(tile, spriteCanvas, seed) {
        if (!spriteCanvas) {
            ...
            currentCtx.clearRect(
                0,
                0,
                spriteCanvas.c.width,
                spriteCanvas.c.height,
            );
        }
    ...
    function drawSprite(x, y, tile) {
        ...
        if (mapOverrides[x][y]) {
            mapOverrides[x][y].fn.call(mapOverrides[x][y], currentDrawPos, [
                x,
                y,
            ]);
        }
        ...
        if (tile === "water" && 1000 / time > 30) {
            ctx.drawImage(
                spriteCanvas.c,
                currentDrawPos[0] - tileSize / 2,
                currentDrawPos[1] -
                    tileSize * 1.5 -
                    Math.sin(x + y + now / 200) * 2,
            );
        } else {
            ctx.drawImage(
                spriteCanvas.c,
                currentDrawPos[0] - tileSize / 2,
                currentDrawPos[1] - tileSize * 1.5,
            );
        }
    ...
    function drawTileQueue() {
        ...
        if (tileQueueReset) {
            drawTileQueueOffset =
                (1 - (now - tileQueueReset) / 200) *
                (getTileQueuePos(1) - getTileQueuePos(0));
        ...
        if (!opts.renderOnly) {
            ...
            if (renderChrome) {
                if (gameIsFree && selectedTile && lastHoveredTilePos) {
                    ...
                    ctx.drawImage(
                        spriteCache.ok.c,
                        lastHoveredTilePos[0] - tileSize / 2 + viewport[0],
                        lastHoveredTilePos[1] - tileSize * 1.5 + viewport[1],
                    );
                    ...
                    if (isTouching) {
                        ...
                        if (
                            lastHoveredTilePos &&
                            map[lastHoveredTileCoords[0]] &&
                            map[lastHoveredTileCoords[0]][
                                lastHoveredTileCoords[1]
                            ]
                        ) {
                            var indicatorTileName =
                                lastHoveredTileType === "helipad" ||
                                canPlaceTileHere(
                                    selectedTile,
                                    lastHoveredTileCoords,
                                )
                                    ? "ok"
                                    : "notok";
                            ctx.drawImage(
                                spriteCache[indicatorTileName].c,
                                lastHoveredTilePos[0] -
                                    tileSize / 2 +
                                    viewport[0],
                                lastHoveredTilePos[1] -
                                    tileSize * 1.5 +
                                    viewport[1],
                            );
                        }
