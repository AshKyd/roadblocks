import saveDialog from "../templates/savedialog.js";
var d = (window.d = document);
var w = (window.w = window);
import bootstrap from "../templates/bootstrap.js";
d.body.innerHTML = bootstrap;
...
var actions = {
    ...
    "Free map": function () {
        if (!Storage.state.Puzzle5) {
            ...
            tileList.innerHTML = SpriteLib.placeable
                .map(function (sprite) {
                    return (
                        '<img id="t' +
                        sprite +
...
function showMenu() {
    ...
    d.querySelector("#menu").innerHTML = menuOptions
        .map(function (text) {
            ...
            return (
                ...
                dac +
                ' src="' +
                drawTile(text[1], Math.min(canvas.width, canvas.height) / 4) +
                '">' +
                text[2] +
                "</div>"
            );
    ...
    if (window.AudioContext || window.webkitAudioContext) {
        ...
        modal.show(
            i18n.t("This browser is too old to run Road Blocks.") +
                '<a class="pill active" href="http://spacekidgames.com/road-blocks/system-requirements">' +
