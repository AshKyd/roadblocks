...
Object.keys(levels).map(function(levelType){
    levels[levelType] = levels[levelType].map(function(level){
        ...
        return {
            ...
            strict: data[6],
            dist: data[7] ? data[7].split('').map(numberise) : undefined,
            predef : predef,
            intro: level[1],
        };
    });
